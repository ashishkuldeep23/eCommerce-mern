   ...
   try {
      ...
      if (!allRequiredField) {
         ...
         return res
            ...
            .send({
               status: false,
               message: `Mandatory feild is not given. | ${allRequiredField}`,
            });
      }
      ...
      if (!cartData || cartData.length === 0) {
         return res
            .status(400)
            .send({ status: false, message: `Card data not provided` });
      }
      ...
      let mailOptions = sendMailWithNodemailerFormate(
         userData.email,
         "Thank you for shopping with us. Check your order details.",
         makeHtmlForOrders(req.body, "unsubcribe_URL"),
      );
      ...
      cartData.map(async (data, i) => {
         ...
         // // // TODO : Need to update this logic (That will be more better than this) ---->
            ...
            findActualProduct.type.map(async (type, i) => {
               if (type.typeId === data.type.typeId) {
                  ...
                  type = {
                     ...
                     typeVerity: type.typeVerity,
                     typeId: type.typeId,
                     typeStock: reduceStock,
                     typePrice: data.type.typePrice,
                  };
               }
   ...
   try {
      ...
      if (!whatUpdate)
         return res
            ...
            .send({
               status: false,
               message: "Metion what you want to update.",
            });

      if (!orderId || !uuid.validate(orderId))
         return res
            .status(400)
            .send({
               status: false,
               message: "OrderId is not given or invalid orderId.",
            });
      ...
      if (!findOrder)
         return res
            ...
            .send({
               status: false,
               message: "No order data found with given ID.",
            });
