...
const {
   transport,
   sendMailWithNodemailerFormate,
   makeHtmlMailForVerifyEmail,
...
async function creteUserControllor(req, res) {
   try {
      ...
      if (req.files.length > 0) {
         ...
         let result = await uploadImageOnCloudinary(
            filePathIs,
            "users_Imgs_Ecom",
         );
      ...
      let mailOptions = sendMailWithNodemailerFormate(
         ...
         makeHtmlMailForVerifyEmail(
            `${process.env.BACKEND_URL}/verifyMail?token=${createNewUser.verifyMailToken}&email=${createNewUser.email}`,
         ),
      );
...
async function updateUser(req, res) {
   try {
      ...
      if (whatUpadte === "address") {
         ...
         let index = findUserData.address.findIndex(
      ...
      res.status(200).send({
         status: true,
         message: `${whatUpadte} of ${upadtedUser.firstName} gets Update`,
         data: upadtedUser,
      });
   } catch (err) {
      console.log(err.message);
...
async function verifyMailReq(req, res) {
   try {
      ...
      if (userFoundWithMail.verifyMailToken)
         return res
            .status(400)
            .send({ status: false, message: "Mail id is already verified." });
      ...
      await userModel.findOneAndUpdate(
         { email: userFoundWithMail.email },
         {
            $set: {
               verifyMailToken: verifyMailToken,
               verifyMailTokenExpiree: Date.now() + 3600000,
            },
         },
         { new: true, upsert: true },
      );
      ...
      res.status(200).send({
         status: true,
         message: "Mail sended sucessfull, check you mail inbox now.",
      });
   } catch (err) {
      console.log(err.message);
      return res
...
async function verifyMailController(req, res) {
   try {
      ...
      if (findUser.isEmailVerified) {
         return res
            .status(400)
            .send({ status: false, message: "Your mail is already verified." });
      }
...
async function checkUserPesentWithMail(email) {
   try {
      ...
      let checkingUser = await userModel
         .findOne({ email: email })
         .select(
            "-_id -password -verifyMailToken -resetPasswordToken -__v -createdAt -updatedAt",
         );
...
async function forgotReqHandler(req, res) {
   try {
      ...
      if (!email)
         return res
            .status(400)
            .send({ status: false, message: "Please provide email of user." });
      ...
      await userModel.findOneAndUpdate(
         { email: userFoundWithMail.email },
         {
            $set: {
               resetPasswordToken: resetPassToken,
               resetPasswordTokenExpiree: Date.now() + 3600000,
            },
         },
         { new: true, upsert: true },
      );
...
async function forgotMainHandler(req, res) {
   try {
      ...
      if (!email || !token || !password)
         return res
            .status(400)
            .send({ status: false, message: "Imp. feilds not given." });
      ...
      res.status(200).send({
         status: true,
         message: "New password saved successfull.",
         data: getUser.email,
      });
...
async function userWithEmail(req, res) {
   ...
   if (!checkUser)
      return res
         .status(400)
         .send({ status: false, message: "Not user found with given email." });

   return res
      .status(200)
      .send({ status: true, message: "Data found with given email." });
}
...
async function bugReportHandler(req, res) {
   try {
      ...
      if (!validateEmail.test(email))
         return res
            .status(400)
            .send({ status: false, message: "Email in not valid." });
...
async function addOrRemoveWishList(req, res) {
   try {
      ...
      if (!productId) {
         return res
            .status(400)
            .send({ status: false, message: "Product id is not given." });
      }
      ...
      if (!userData.wishList || userData.wishList.length === 0) {
         ...
         let indexOfWishList = userData.wishList.indexOf(
            productDataById._id.toString(),
         );
      ...
      // // // Updated User data ---->
      const sendUserData = await userModel
         .findById(updatedUserData._id)
         .populate({
            path: "wishList", // // // Here we should give the feild name of data ----->
