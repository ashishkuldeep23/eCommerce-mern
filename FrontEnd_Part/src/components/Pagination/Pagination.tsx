...
export default function Pagination() {
   ...
   const totalProducts = useSelector(
      (state: RootState) => state.allProductWithCatReducer.totalProducts,
   );

   const onePageLimitValue = useSelector(
   ...
   const sortByPrice = useSelector(
      (state: RootState) => state.allProductWithCatReducer.sortByPrice,
   );
   ...
   function handlePagination(
      ...
      if (page < 1) {
         toast.error(
            `Page ${page} is not possiable for search | 400 (Bad Request)`,
         );
         return;
      }
   ...
   return (
      <div>
         {totalPageShow <= 0 && (
            <p className="text-end pr-5 ">
               Getting actual data...(don't use pagination now)
            </p>
         )}
         ...
         <div className="flex items-center justify-between border-t border-gray-200  px-4 py-3 sm:px-6">
            ...
            <div className="flex flex-1 justify-between items-center sm:hidden">
               <p
                  ...
                  onClick={(e) => {
                     handlePagination(e, currentPage - 1);
                  }}>
                  Previous
               </p>
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
               <div>
                  <p className="text-sm ">
                     ...
                     <span className="font-medium">{totalPageShow || 1}</span>{" "}
                     results
                  </p>
               </div>
               <div>
                  <nav
                     ...
                     {totalPageShow > 0
                        ? Array.from(Array(totalPageShow), (_, i) => {
                             return (
                                <p
                                   key={i}
