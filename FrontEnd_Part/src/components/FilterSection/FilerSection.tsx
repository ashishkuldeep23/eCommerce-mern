...
export default function FilterSection({ children }: any) {
   ...
   const [newFilter, setNewFilter] = useState<FilterArr>([]);
   ...
   const allProductWithCatReducer = useSelector(
      (store: RootState) => store.allProductWithCatReducer,
   );
   const allCategory = allProductWithCatReducer.filterAllCateory;
   const allBrands = allProductWithCatReducer.filterAllBrands;
   const sortByPrice = allProductWithCatReducer.sortByPrice;
   const limitValue = allProductWithCatReducer.onePageLimit;
   const { brand, category } = allProductWithCatReducer.searchBrandAndCate;

   function onChngeHandlerOfFilter(
      e: EventTypeOfOncahnge,
      section: string,
      value: string,
   ) {
      e.stopPropagation();
      ...
      // // // Set filter section hidden -------->>
      let latestfilter = newFilter.map((sec) => {
         if (sec.id === section) {
            let newOption = sec.options.map((opt) => {
               if (opt.value === value) {
      ...
      setNewFilter(latestfilter);

      setMobileFiltersOpen(false); // // // It will make filter modal section hidden.
   ...
   function onClickHandlerForPrice(e: ClickEventForPrice, price: String) {
      ...
      if (price === "acc") {
         ...
         dispatch(
            fetchAllProducts({ brand: brand, category: category, price: "1" }),
         );
      }
      ...
      /// // // Move to product listing div ----------->>
      let productListingDiv = document.querySelector("#product_listing_div");
      window.scrollTo({
         top: (productListingDiv?.getBoundingClientRect().top || 500) + 50,
         behavior: "smooth",
      });
   ...
   useEffect(() => {
      ...
      if (
         ...
         // // // Making Formate only --->
         let makeAllBrands = allBrands.map((brand) => {
         ...
         // // // Making Formate only --->
         let makeAllCategory = allCategory.map((cate) => {
            return { value: cate, label: cate, checked: false };
         });
         ...
         setNewFilter([
            { id: "category", name: "Category", options: makeAllCategory },
            { id: "brand", name: "Brand", options: makeAllBrands },
         ]);
      }
   }, [allCategory, allBrands]);

   return (
      <div
         className={`${!themeMode ? "bg-white text-gray-600 " : "bg-black text-gray-300 "} `}>
               ...
               <Dialog
                  ...
                  <div className="fixed inset-0 z-40 flex">
                     <Transition.Child
                        ...
                        <Dialog.Panel
                           className={`  w-[50%] z-50 relative ml-auto flex h-full flex-col overflow-y-auto  py-4 pb-12 shadow-xl ${!themeMode ? "bg-white text-gray-600 " : "bg-black text-gray-300 "} `}>
                           <div className="flex items-center justify-between px-4">
                              ...
                              <button
                                 type="button"
                                 className="-mr-5 mt-2 flex mx-auto h-10 w-10 items-center justify-center rounded-md  p-2 "
                           ...
                           {/* <form className="mt-4 border-t border-green-300">
                              {newFilter.map((section) => (
                                 <Disclosure
                                    as="div"
                                    key={section.id}
                                    className="border-t border-green-300 px-4 py-6">
                                    {({ open }) => (
                                       <>
                           ...
                           {/* <p>Ok button, check now</p> */}
                           <FilterForm
                              className=" !block !mt-4 !mx-2 !border-t !border-green-300"
                              newFilter={newFilter}
                              onChngeHandlerOfFilter={onChngeHandlerOfFilter}
                           />
                        </Dialog.Panel>
                     </Transition.Child>
                  </div>
            ...
            <main className="mx-auto max-w-full md:max-w-allAk px-1 md:px-2 lg:px-8  p-2 pt-16 ">
                  ...
                  <div className="flex items-center">
                     <Menu as="div" className="relative inline-block text-left">
                        ...
                        <Transition
                           ...
                           <Menu.Items
                              className={`absolute -right-1/2 z-10 mt-2 w-40 origin-top-right rounded-md shadow-2xl ring-3 ring-blue-700 ring-opacity-5 focus:outline-none ${!themeMode ? "bg-white text-gray-600 " : "bg-gray-800 text-gray-300 "} `}>
                              <div className="py-1">
                     ...
                     <button
                        type="button"
                        className="-m-2 ml-1 p-2  hover:scale-105 sm:ml-6 lg:hidden"
                     ...
                     {(brand || category) && (
                        <button
                           onClick={clearFilter}
                           className=" inline lg:hidden  text-base bg-red-500 mx-1 px-1.5 text-white rounded">
                           X
                        </button>
                     )}
                  </div>
               ...
               <section
                  ...
                  <div className="grid grid-cols-1  gap-y-10 lg:grid-cols-6">
                     <FilterForm
                        ...
                        clearAllBtnWithAll={
                           (brand || category) && (
                              <button
                                 onClick={clearFilter}
                                 className=" text-base bg-red-500  tracking-wider px-2 text-white rounded hidden lg:inline">
                                 Clear All
                              </button>
                           )
                        }
                     ...
                     {/* Product grid */}
                     <div id="product_listing_div" className="lg:col-span-5">
