...
export const AllOdersComp = () => {
   ...
   return (
      <>
         ...
         <div
            className={` my-20 w-full smm:w-10/12 sm:w-3/4  md:w-10/12  py-2 px-1.5 rounded border 
                ${
                   themeMode
            ...
            <div className=" flex flex-col">
               <h1 className=" underline font-semibold text-3xl text-center">
                  All Orders details ({allOrders.length}){" "}
               </h1>
               ...
               <button
                  ...
                  Get {sortBy === "-1" ? "Oldest" : "Newest"} first
               </button>
            </div>

            <ul
               role="list"
               className={`divide-y ${themeMode ? " divide-slate-100 " : "divide-slate-900"} `}>
               {allOrders.length > 0 ? (
                  allOrders.map((ele, i) => {
                     return (
                        <SingleAdminOrder
...
function SingleAdminOrder({ order, i }: { order: AdminAllOrders; i: number }) {
   ...
   const [arrOfOptions, setArrOfOptions] = useState<
      [ValueOfOrderStatus, ValueOfOrderStatus]
   >(["Pending", "Received"]);
   ...
   function orderSelectChanged(e: React.ChangeEvent<HTMLSelectElement>) {
      ...
      let bodyOrReq = {
         body: {
            whatUpdate: "statusAdmin",
            orderId: order.id,
            makeOrderStatus: e.target.value,
         },
      };
   ...
   return (
      <>
         <li className="flex flex-col justify-between gap-x-6 py-5 relative">
            <p
            ...
            <div className=" w-full flex flex-col items-center md:px-20">
               <div className=" w-full flex justify-between flex-col items-center md:flex-row">
                  <div>
                     ...
                     <p>
                        Where : <span>{order.address.street}</span>,{" "}
               ...
               <p>{new Date(order.whenCreated).toLocaleString()}</p>
               ...
               <button
                  ...
                  {" "}
                  {!seeDetails ? "Show" : "Hide"} ordered items
               </button>
            </div>
               ...
               {seeDetails && (
                  <div className=" flex flex-col items-center">
                     <p className=" border-b">
                        Item{order.cartData.length > 1 ? "s Are :-" : " Is :-"}
                     </p>
