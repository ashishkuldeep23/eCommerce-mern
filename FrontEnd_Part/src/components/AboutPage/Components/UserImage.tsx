...
const UserImageDiv = ({ userImgClass }: { userImgClass?: string }) => {
   ...
   return (
      <>
         <div>
            <div
               className={`w-auto relative ${userImgClass}`}
               onClick={() => {
                  showModalWithValues();
               }}>
               {getUserData &&
               (getUserData.firstName !== "" || getUserData.lastName !== "") ? (
                  <img
                     className={`w-80 md:max-w-sm  rounded-xl  outline outline-offset-2 outline-emerald-300  hover:outline-offset-4  transition-all ${userImgClass} `}
                     src={userImage}
                     alt=""
                  />
               ) : (
                  <div
                     className={` w-full h-56 sm:w-80 sm:h-80 md:max-w-sm  rounded-xl  outline outline-offset-2 outline-emerald-300 bg-slate-300  hover:outline-offset-4  transition-all ${userImgClass} `}></div>
               )}

               <label
                  className=" absolute bg-green-400 text-white -right-2 -bottom-2 px-1 rounded-full"
               ...
               {getUserData.isEmailVerified ? (
                  <div className="mt-3 flex flex-col justify-start items-end">
                     ...
                     <div>
                        ...
                        {userImgFile && (
                           <button
                              className="bg-green-500 px-1 rounded text-white font-bold"
                              onClick={(e) => {
                                 e.stopPropagation();
                                 submitNewImg();
                              }}>
