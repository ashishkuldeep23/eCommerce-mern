                        ...
                        return (
                            <Fragment key={order.id}>
                                ...
                                <div className=' pb-14 flex justify-center'>
                                    ...
                                    <div className=' w-fit h-fit border-2 border-green-300 sm:pr-3 rounded  flex flex-wrap gap-2 justify-center items-center'>
                                        ...
                                        <div className={`w-full smm:w-72 h-96 border-2 border-green-300 rounded md:max-w-sm px-1 flex flex-col justify-center text-center relative ${!themeMode ? "bg-green-300" : "bg-green-800"} `}>
                                            ...
                                            <p className=' text-3xl font-bold'>{order.fullName}</p>
                                            <div className='flex justify-center text-xl'>
                                                <p>{order.paymentMethod},</p>
                                                <p>{order.phone}</p>
                                            </div>
                                            <p className=''>Address : {order.address.street + " " + order.address.city + " " + order.address.country + " " + order.address.pincode}</p>
                                            <p>Date & Time : {order.whenCreated}</p>
                                            <p>All Orders <span className=' hidden sm:inline'>➡️</span> <span className=' inline sm:hidden'>⬇️</span> </p>
                                        </div>
                                        ...
                                        <div className=' flex flex-wrap justify-evenly gap-2'>

                                            {
                                                ...
                                                order.cartData.map((order: CardDataInter) => <SingleOrderData order={order} />)
...
function SingleOrderData({ order }: { order: CardDataInter }) {
    ...
    return (
        <div
            key={order.id}
            className="w-full smm:w-72 flex flex-col items-center hover:cursor-pointer"
        ...
        >
            ...
            <img className=" w-72 h-64 object-cover md:max-w-sm rounded" src={order.thumbnail} ></img>

            {/* <div className=" w-full sm:w-72  h-56 sm:h-72 bg-slate-300 rounded md:max-w-sm"></div> */}

            <p className={` ${!themeMode ? "bg-slate-300" : "bg-slate-700"} h-6 w-full rounded my-0.5 px-1 mt-4`}>{order.title}</p>
            <p className={`${!themeMode ? "bg-slate-300" : "bg-slate-700"} h-6 w-full rounded my-0.5 px-1 font-bold`}>₹{makeMoreRaedablePrice(order.price)}</p>
            {
                order.type
                &&
                // ""
                <p className={`${!themeMode ? "bg-slate-300" : "bg-slate-700"} h-6 w-full rounded my-0.5 px-1 capitalize`}>{order.verity.typeName[0] + " : " + order.verity.typeName[1] + "  " + order.verity.typeVerity[0] + " : " + order.verity.typeVerity[1]}</p>
            }
            {/* <h2 className="bg-slate-300 h-6 w-full rounded my-0.5 px-1">Date and Time </h2> */}
        </div>
    )
