...
const PaymentComp = () => {
   ...
   const getTotalItemsOfCart = useSelector(
   ...
   const onSubmit: SubmitHandler<OrderData> = (data) => {
      ...
      if (
         !data.address ||
         (data.address && Object.keys(data.address).length < 3)
      ) {
         setError("address", {
            type: "manual",
            message: "Please Add or Choose addres.",
         });
         return;
      }
   ...
   return (
      <>
         <div
            className={` h-allAk flex flex-col  overflow-y-scroll shadow-xl ${!themeMode ? "bg-white text-gray-900" : "bg-black text-gray-200"}  `}>
            ...
            <h1 className=" font-bold text-5xl">Total Price is : {getTotalItemsOfCart.length}</h1> */}
                  ...
                  <h1 className="mt-5 bg-emerald-400 px-3 rounded text-white text-2xl font-bold">
                     Place Order 🛒
                  </h1>
                  ...
                  <div className="flex flex-col items-center justify-center w-[85%] mt-10">
                     <div
                        ...
                        <p
                           className={`  ${!themeMode ? "bg-slate-100" : "bg-slate-900"}   rounded my-0.5 px-1 `}>
                        ...
                        <div
                           ...
                           {getUserData.address &&
                              getUserData.address?.length === 0 && (
                                 <p className={`px-1 text-center `}>
                                    Note : Please add address, this is improtant
                                    in order data.{" "}
                                 </p>
                              )}
                           ...
                           {getUserData.address &&
                              getUserData.address?.length > 0 && (
                                 <p
                                    className={`${!themeMode ? "bg-slate-100" : "bg-slate-900"} px-1 `}>
                                    Address :
                                 </p>
                              )}
                  ...
                  <form
                     noValidate
                     className={`space-y-3 w-full flex flex-col items-center relative ${getOrderData.isLoading && "opacity-50"} `}
                     ...
                     <div className=" absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-100 scale-150 z-10">
                        {getOrderData.isLoading && (
                           <div role="status">
                              <svg
                                 aria-hidden="true"
                                 className="inline w-40 h-40  text-transparent animate-spin fill-blue-600 opacity-100 "
                                 viewBox="0 0 100 101"
                                 fill="none">
                                 <path
                     ...
                     <div className=" w-full smm:w-4/5 border-b border-gray-900/10 pb-12">
                        <div
                           className={`grid grid-cols-1 gap-x-6  gap-y-5 sm:grid-cols-6  border border-teal-500 p-5 rounded-xl ${!themeMode ? "bg-teal-50" : "bg-teal-950"}  `}>
                           <h2 className="font-semibold leading-7 text-3xl text-center underline  col-span-full ">
                              Order Details
                           </h2>
                                 ...
                                 <input
                                    ...
                                    {...register("phone", {
                                       ...
                                       pattern: {
                                          value: /(\+)?(91)?( )?[789]\d{9}/gi,
                                          message:
                                             "Must be valid Indan phone number.Start with 7 or 8 or 9.",
                                       },
                                    })}
                                    id="phone"
                                    className={`block w-full rounded-md border-0 py-1.5 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 ${!themeMode ? " bg-white text-gray-900 " : "bg-gray-900 text-white"} `}
                           ...
                           <div className="flex flex-col items-start justify-center  md:flex-row col-span-full relative border-b border-teal-500 pb-2 gap-2 sm:gap-5 ">
                              {/* <AddressInputCopm /> */}
                              ...
                              <div className=" md:w-1/2">
                                 ...
                                 getUserData.address.length > 0 ? (
                                    ...
                                    <div>
                                       <p
                                          className={`${!themeMode ? "bg-slate-100" : "bg-slate-900"} relative px-1 border-b rounded-b-md `}>
                                          No address found
                                       </p>
                                       <p
                                          className={`${!themeMode ? "bg-slate-100" : "bg-slate-900"} relative px-1 border-b rounded-b-md `}>
                                          Go to top and add new Address
                                       </p>
                                 ...
                                 <p className="text-sm pl-2 text-red-500 font-bold text-end">
                                    {" "}
                                          ...
                                          <div className="flex items-center gap-x-3">
                                             <input
                                                ...
                                                {...register("paymentMethod", {
                                                   required:
                                                      "Payment method number not given.",
                                                })}
                                                type="radio"
                           ...
                           <div className=" col-span-full flex justify-center flex-col items-center hover:cursor-pointer ">
                              <div className="flex justify-around w-full mb-4">
                                 <p>
                                    Total items : {getTotalItemsOfCart.length}
                                 </p>
                                 <p>
                                    Total Price : ₹
                                    {makeMoreRaedablePrice(getTotalPriceOfCart)}
                                 </p>
                              </div>
