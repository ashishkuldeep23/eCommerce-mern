...
const ReviewDivBoth = forwardRef<Ref, Props>(({ data }, ref) => {
  ...
  const isLoading = reviewState().isLoading;
  ...
  return (
    <>
      <div
        ...
        <h3 className="text-3xl font-bold underline lg:text-center ">
          Create new Review
        </h3>
        ...
        <div className="mt-10">
          ...
          {singleProductData?.review &&
            singleProductData?.review?.length > 0 && (
              <p className=" mt-5 text-center">
                *Use double click to like reviews (like in insta)
              </p>
            )}
          ...
          <div className="flex flex-wrap my-5 lg:justify-center">
            ...
            singleProductData?.review &&
            singleProductData?.review?.length > 0
              ? singleProductData?.review?.map((reviewData: ReviewData) => (
                  <SingleReview reviewData={reviewData} key={reviewData.id} />
                ))
...

          {
            ...
            singleProductData?.review &&
              singleProductData?.review?.length > 0 && (
                <div>
                  <p className=" underline block text-center">
                    {" "}
                    Total Stars{" "}
                    <strong>{singleProductData.rating.avgRating}</strong> |
                    Total Reviews{" "}
                    <strong>{singleProductData.rating.totalPerson}</strong> |
                    Avg Rating{" "}
                    <strong>
                      {(
