...
const PostReview = () => {
   ...
   const inputReviewData = useSelector(
      (state: RootState) => state.reviewReducer.inputReviewData,
   );
   ...
   const reviewUpdateState = useSelector(
      (store: RootState) => store.reviewReducer.isUpdatng,
   );
   ...
   function postReviewHandler() {
      ...
      dispatch(
         createNewReview({
            comment: inputReviewData.comment,
            stars: inputReviewData.stars,
   ...
   function submitEmoji(
      ...
      dispatch(
         setReviewData({
            data: {
               ...inputReviewData,
               comment: inputReviewData.comment + `${emoji}`,
            },
         }),
      );
   }
      ...
      <>
         {
            ...
            singleProductData && singleProductData.images.length > 0 ? (
               <div
                  className={`mt-5 my-4 border p-2 sm:px-3 rounded border-green-300 relative ${!themeMode ? "bg-blue-50" : "bg-blue-950"} `}>
                  {/* Loader code -------> */}
                  ...
                  <textarea
                    ...
                    //  cols={25}
                     rows={2}
                     className={` w-44 xsm:w-56 resize-none rounded p-1 ${!themeMode ? " bg-white text-gray-900 border-black " : "bg-gray-900 text-white border-white"} border-black z-10 `}></textarea>

                  {/* Emojies Div ----> */}
                  <div
                     className={`border  rounded-b -mt-2 mb-1 flex flex-wrap justify-center ${!themeMode ? "bg-white border-black" : "bg-black border-white"} flex justify-between `}>
                     <p
                        className=" opacity-80 hover:cursor-pointer hover:bg-blue-400"
                        onClick={(e) => {
                           submitEmoji(e, "👍");
                        }}>
                        👍
               ...
               <div
                  className={` w-60 h-32 mt-5 border p-2 px-3 rounded border-green-300 relative ${!themeMode ? "bg-blue-50" : "bg-blue-950"}  ${isLoading && " opacity-50"} `}></div>
            )
         }
      </>
   );
