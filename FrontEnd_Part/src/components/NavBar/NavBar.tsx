...
export default function NavBar() {
   ...
   return (
      <Disclosure
         as="nav"
         className="bg-gray-800 sticky -top-16 md:top-0 z-40  ">
         {({ open }) => (
            <>
   ...
   return (
      <>
         <div
            ...
            <Link to={"/admin"}>
               <button className=" px-4 capitalize font-bold text-sm sm:text-lg">
                  Go To Admin Page
               </button>
            </Link>
         </div>
...
function MenuOfTabAndAbove({
   gotoHomeBtnClickHandler,
}: {
   gotoHomeBtnClickHandler: Function;
}) {
   const userData = userState().userData;
   const navigate = useNavigate();
...
// // // {/* This div will visiable on less then md devices main div. (This div includes Icon and open & close icon) */ }
function MobileUICodeLeftSection({
   open,
   gotoHomeBtnClickHandler,
}: {
   open: boolean;
...
function MainSearchBarWithLogics() {
   ...
   const removeOneKeywordFromHistory = (product: IProduct) => {
      ...
      let newHistoryArr = searchHistory.filter(
   ...
   return (
      <>
         ...
         {showSuggestion && (
            <div
               className={` ${!themeSate ? "bg-white text-black" : " bg-gray-900 text-white"} w-full absolute top-full mt-0.5  rounded-b-md py-2 px-1 z-20 `}>
               <ul className="px-1 pb-1 max-h-96 overflow-y-scroll border rounded border-green-300">
                  ...
                  {text && (
                     <div
                        className="p-1 text-white bg-red-500 m-1 font-bold rounded flex justify-between"
                        onClick={() => {
                           setText("");
                        }}>
                        <p>Close</p>
                  ...
                  {isLoading && <span id="search_loader"></span>}

                  {allProduct && allProduct.length >= 0 && text === "" ? (
                     <h1 className="my-5 text-center text-danger fw-bold">
                        Search product by it's <strong>Name</strong> or{" "}
                        <strong>Category</strong> or <strong>Brand name</strong>
                        .
                     </h1>
                           ...
                           return (
                              <div
                                 ...
                                 <span
                                    ...
                                    onClick={(e) => {
                                       e.stopPropagation();
                                       removeOneKeywordFromHistory(ele);
                                    }}>
                                    X
                                 </span>
                              </div>
...
function UniversalLoaderAndScroll() {
   ...
   const orderIsLoading = useSelector(
      (state: RootState) => state.orderReducer.isLoading,
   );
   const reviewIsLoading = useSelector(
      (state: RootState) => state.reviewReducer.isLoading,
   );
   ...
   function leftClickhandler(
      ...
      let aboveScrollDivLeft = document
         .getElementById("scroll_percent_indi_top")
         ?.getClientRects()[0].x;
   ...
   if (productIsLoading || orderIsLoading || reviewIsLoading || userIsLoading) {
      return (
         <>
            <div
               style={{ height: "1px" }}
               className=" w-full bg-blue-200 flex items-center relative">
      ...
      <>
         <div className="w-full h-1.5 relative bg-gray-800 border-b ">
            <div
               ...
               style={{
                  left: `${left}%`,
                  width: `${Math.round(windowHight / 45)}%`,
               }}
